...
H3Handle               H3_Init(SH3InitParams params); //Initiate Engine
void                   H3_Terminate(H3Handle* h3); //Terminate Engine

float                  H3_GetTime(); 
float                  H3_GetDeltaTime();

bool                   H3_Input_IsKeyDown(EH3Key k); //Check if a key is down
bool                   H3_Input_IsKeyPressed(EH3Key k); //Check if a key is pressed
void                   H3_Input_GetMousePos(H3Handle h3, int* x, int* y); //Get Mouse Position
bool                   H3_Input_IsMouseBtnDown(EH3MouseButton btn); //Check if the mouse's button is down
bool                   H3_Input_IsMouseBtnPressed(EH3MouseButton btn); //Check if the mouse's button is pressed

H3Handle               H3_Scene_Create(H3Handle h3, bool physicsLocksRotation); //Create Scene
void                   H3_Scene_Destroy(H3Handle* scene); //Destroy Scene
void                   H3_Scene_SetGravity(H3Handle scene, float x, float y);

H3Handle               H3_Object_Create(H3Handle scene, const char* objName, H3Handle parent); //Create Object
void                   H3_Object_Destroy(H3Handle* object, bool recursive); //Destroy Object
...
void                   H3_Object_Rotate(H3Handle object, float angle); //Rotate the object
void                   H3_Object_Scale(H3Handle object, float s); //Increase or Reduce the object's size
void                   H3_Object_SetTranslation(H3Handle object, float x, float y); //Set the object's position
